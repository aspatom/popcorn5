...
static int status_connection = 0;
...
void tcp_echoclient_connect_sustained(void);
static err_t tcp_echoclient_connected_sustained(void *arg, struct tcp_pcb *tpcb, err_t err);
...
static err_t tcp_echoclient_connected(void *arg, struct tcp_pcb *tpcb, err_t err)
{
  ...
  if (err == ERR_OK) {
    ...
    if (es != NULL) {
      ...
      /* allocate pbuf */
      es->p_tx = pbuf_alloc(PBUF_TRANSPORT, strlen((char*)post_packet) , PBUF_POOL);
         
      if (es->p_tx) {       
        //callback function initialize. 

        /* copy data to pbuf */
        pbuf_take(es->p_tx, (char*)post_packet, strlen((char*)post_packet));
...
void tcp_echoclient_connect_sustained(void)
{
  ...
  if (echoclient_pcb != NULL) {
    ...
    /* connect to destination address/port */
    tcp_connect(echoclient_pcb,&DestIPaddr,DEST_PORT,tcp_echoclient_connected_sustained);
  ...
  if (err == ERR_OK) {
    ...
    if (es != NULL) {
			if(status_connection == 0)
			{
				...
				sprintf((char*)data, "POST /check HTTP/1.1\r\nHost: 192.168.1.27\r\n\r\n");
				//sprintf((char*)data, "POST /gcm/sendErrAlert/306 HTTP/1.1\r\nHost: product.checkup-api.co.kr\r\n\r\n");
			...
			USART3_PutString("ERROR >> Connection close e1\r\n");
      /* close connection */
      tcp_echoclient_connection_close(tpcb, es);
      status_connection = 0;
      /* return memory allocation error */
      return ERR_MEM;  
			...
			USART3_PutString("ERROR >> Connection close e2\r\n");
    /* close connection */
    tcp_echoclient_connection_close(tpcb, es);
		status_connection = 0;
  }
